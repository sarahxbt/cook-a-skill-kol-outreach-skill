...
|---|---|---|---|
| 1 | Claude Sonnet 4.6 (Claude Desktop) | Brainstorm ý tưởng, viết spec draft đầu tiên | Fast creative ideation with strong context retention — Sonnet excels at generating multiple concepts quickly and structuring a first draft from a vague brief. |
| 2 | Claude Sonnet 4.6 (Claude Desktop) | Viết review prompts cho AI khác chạy (prompt engineering) | Same model to maintain consistency — Sonnet understands its own blind spots well enough to write review prompts that target them. Self-critique identified 6 weaknesses before external review. |
| 3 | Gemini 3.1 Pro (qua Perplexity web UI) | Independent cold review spec.md — không bias từ Claude | Large context window for ingesting the entire spec + guidebook in one shot. Zero exposure to earlier Claude conversations = genuinely independent review. Found blind spots Claude missed. |
| 4 | GPT-5.2 Thinking (qua Perplexity web UI) | Apply patches chính xác từ review findings | GPT Thinking mode excels at precise, surgical edits — it follows patch instructions literally without over-correcting or rewriting surrounding context. Best for "change X to Y" tasks. |
| 5 | Claude Sonnet 4.6 (Claude Desktop) | Review lại sau khi GPT apply patches | Sonnet re-reads the patched version fresh, verifying that GPT's fixes didn't introduce new inconsistencies. Familiar with original design intent → catches semantic drift. |
| 6 | Gemini 3.1 Pro (qua Perplexity web UI) | Review SKILL.md lần 2 (independent review) | Second independent review using the same unbiased model. Ensures the system prompt (SKILL.md) aligns with the spec — different artifact, same standard of scrutiny. |
